...
DF3 <- df.list[[r]]
par(oma=c(0,0,0,0), mar=c(3,4.5,1,0), bg=alpha("white", 0.85))
x <- barplot(DF3[,"extra_PA_AGB"], xaxt="n",# names.arg=row.names(DF3), las=2,
        xlim=c(0,12), ylim=c(0,2.1),
        #main=paste(regions[r]),
        col=mypalette2[DF3[,"colors"]],
        cex.axis=1.25, cex.names=1.25)
labs <- paste(row.names(DF3))
text(cex=0.25, x=x-.25, y=-0.5, labs, xpd=TRUE, srt=45)
title(ylab="AGB (Gt)", line=3, cex.lab=0.3)
text(5, 1.9, paste(regions[r]), cex=0.35)
...
barfigs.list[[r]] <- image_graph(width = width_bp, height = height_bp, res = res_bp)
DF3 <- df.list[[r]]
par(oma=c(0,0,0,0), mar=c(3,4,1,0), bg=alpha("white", 0.85))
x <- barplot(DF3[,"extra_PA_AGB"], xaxt="n",# names.arg=row.names(DF3), las=2,
...
labs <- paste(row.names(DF3))
text(cex=0.25, x=x-.25, y=-0.5, labs, xpd=TRUE, srt=45)
title(ylab="AGB (Gt)", line=2.5, cex.lab=0.3)
text(5, sum(DF3[1,"extra_PA_AGB"],DF3[1,"extra_PA_AGB_err"]), paste(regions[r]), cex=0.34)
x <- barplot(DF3[,"extra_PA_AGB"], plot=F)
y <- DF3[,"extra_PA_AGB"]
se <- DF3[,"extra_PA_AGB_err"]
arrows(x, y+se, x, y-se, angle=90, code=3, length=0.05)
...
for(r in 3:5){
  ...
  title(ylab="AGB (Gt)", line=3, cex.lab=1.25)
  text(5, 1.9, paste(regions[r]), cex=1.35)
  x <- barplot(DF3[,"extra_PA_AGB"], plot=F)
  y <- DF3[,"extra_PA_AGB"]
  se <- DF3[,"extra_PA_AGB_err"]
  arrows(x, y+se, x, y-se, angle=90, code=3, length=0.05)
...
text(cex=0.25, x=x-.25, y=-0.5, labs, xpd=TRUE, srt=45)
title(ylab="AGB (Gt)", line=2.5, cex.lab=0.3)
text(5, 1.02*sum(DF3[1,"extra_PA_AGB"],DF3[1,"extra_PA_AGB_err"]), paste(regions[r]), cex=0.34)
x <- barplot(DF3[,"extra_PA_AGB"], plot=F)
y <- DF3[,"extra_PA_AGB"]
se <- DF3[,"extra_PA_AGB_err"]
arrows(x, y+se, x, y-se, angle=90, code=3, length=0.05)
...
image_write(final, path=paste(fig.path,"newFIG3_extraAGB_barplots_2022MAR17_err_updateAPR22.png",sep=""), format="png")
