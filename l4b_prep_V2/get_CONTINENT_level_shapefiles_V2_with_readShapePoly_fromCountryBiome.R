################################################################################
################################################################################
## 5. CONTINENT level shapefiles from Country x Biome files V2
################################################################################
################################################################################
library(tidyr)
library(rgdal)
...
for(i in 1:length(continent_names)){
    ...
    if(length(continent_biome_shp) > 0){
      ...
      if(!is.null(all_pas)){
        pa_layer_name <- paste(continent_names[i],"_PA_samples", sep="")
        pa_write_file_name <- paste(write_f.path,continent_names[i],"_PA_V2.shp", sep="")
      ...
      if(!is.null(all_ctrls)){
        ctrl_layer_name <- paste(continent_names[i],"_Ctrl_samples", sep="")
        ctrl_write_file_name <- paste(write_f.path,continent_names[i],"_Ctrl_V2.shp", sep="")
      ...
      if(!is.null(all_unmatched_pas)){
        unmatch_pa_layer_name <- paste(continent_names[i],"_unmatched_PA_samples", sep="")
        unmatch_pa_write_file_name <- paste(write_f.path,continent_names[i],"_unmatched_PA_V2.shp", sep="")
