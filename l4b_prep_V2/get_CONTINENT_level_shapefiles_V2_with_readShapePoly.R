################################################################################
################################################################################
## 4. CONTINENT level shapefiles from Continent x Biome files V2
################################################################################
################################################################################
library(tidyr)
library(rgdal)
...
write_f.path <- paste(f.path,"/WDPA_AGB_Input_Polygons/Continent_PAs_and_Ctrls_V2/",sep="")
continent_biome_path <- paste(f.path,"WDPA_AGB_Input_Polygons/Continent_Biome_PAs_and_Ctrls_V2/",sep="")
...
#for(i in 1:length(continent_ids)){
i=5

  print(continent_ids[i])
  
    all_pas <- NULL
    all_ctrls <- NULL
    all_unmatched_pas <- NULL
    
    continent_biome_result_path <- paste(f.path,"WDPA_AGB_Input_Polygons/Continent_Biome_PAs_and_Ctrls_V2/",continent_ids[i],"/",sep="")
    continent_biome_shp <- list.files(continent_biome_result_path, pattern=".shp")
    ...
    if(length(continent_biome_shp) > 0){
      ...
      if(!is.null(all_pas)){
        pa_layer_name <- paste(continent_ids[i],"_PA_samples", sep="")
        pa_write_file_name <- paste(write_f.path,continent_ids[i],"_PA_V2.shp", sep="")
      ...
      if(!is.null(all_ctrls)){
        ctrl_layer_name <- paste(continent_ids[i],"_Ctrl_samples", sep="")
        ctrl_write_file_name <- paste(write_f.path,continent_ids[i],"_Ctrl_V2.shp", sep="")
      ...
      if(!is.null(all_unmatched_pas)){
        unmatch_pa_layer_name <- paste(continent_ids[i],"_unmatched_PA_samples", sep="")
        unmatch_pa_write_file_name <- paste(write_f.path,continent_ids[i],"_unmatched_PA_V2.shp", sep="")
