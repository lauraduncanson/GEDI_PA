...
#load("/Volumes/gsdata1/duncansongp/amberliang/trends.Earth/rf_noclimate.RData")
#source("/Volumes/gsdata1/duncansongp/amberliang/trends.Earth/git/GEDI_PA/matching_func.R")
...
#select onkly non-NA matched results
matched_PAs = foreach(this_rds=matched_all, .combine = c, .packages=c('sp','magrittr', 'dplyr','tidyr','raster')) %dopar% {   
  matched_PAs=c()
  # print(this_rds)
...
foreach(this_rds=matched_PAs, .combine = foreach_rbind, .packages=c('sp','magrittr', 'dplyr','tidyr','raster')) %dopar% {
  ...
  if (is.null(matched)==TRUE) {
    ...
    if(table(mras$status[])[2]==0 | table(mras$status[])[1]==0 | is.null(mras)){
      ...
      startTime <- Sys.time()
      iso_matched_gedi <- extract_gedi(matched=matched, mras = mras)  #run filtered csvs on mras for extraction
      ...
      #write.csv(iso_matched_gedi, file=paste(f.path,"WDPA_GEDI_extract4/",iso3,"_wk",gediwk,"/",iso3,"_pa_", id_pa,"_gedi_wk_",gediwk,"_conti_", continent,"_biome_",pabiome,".csv", sep=""))
    }
  }
  return(NULL)
}
  ...
  if (length(table(pa_metrics$status))<2) {
    ...
    pa_stats_summary <- pa_metrics %>%
      ...
      dplyr::summarise(pa_id=na.omit(unique(pa_id)),
                       ...
                       wwfecoreg=getmode(wwfecoreg), REGION=getmode(region) #,PADDD=getmode(PADDD)
                       )%>% 
    ...
    if(!file.exists(paste(f.path,"WDPA_GEDI_extract4/pa_stats/",iso3,"_pa_stats_summary_wk",gediwk,".csv", sep=""))){
      print("not exists")
      write.csv(pa_stats_summary, file=paste(f.path,"WDPA_GEDI_extract4/pa_stats/",iso3,"_pa_stats_summary_wk",gediwk,".csv", sep=""), row.names = F)
...
# fullds0 <- read.csv(paste(f.path,"WDPA_GEDI_extract4/iso_full_nodup/",iso3,"_country_full_nodup_wk",gediwk,".csv", sep=""))
...
# dir.create(paste(f.path,"WDPA_GEDI_extract4/cell_stats/",sep=""))
