...
WDPA_table <- read.csv(paste(f.path,"WDPA_countries_table3.csv",sep=""))
IDs <- as.character(WDPA_table[,"ISO3"])
#for(i in 1:length(IDs)){dir.create(paste(f.path,"WDPA_gedi_l4a/",IDs[i],sep=""))}
for(i in 1:length(IDs)){dir.create(paste(f.path,"WDPA_gedi_l4a_clean/",IDs[i],sep=""))}
################################################################################
## l4a
################################################################################
#startT <- "--start_time 2019-04-18"  ### 2019 day of year 108
#endT <- "--end_time 2020-09-02"  ### 2020 day of year 246
GEDI_data <- "l4a"
...
for(i in 1:length(IDs)){
  ...
  roi_name <- iso3
  outfilePY <- paste(f.path,"WDPA_countries/py_",GEDI_data,"/",iso3,".py",sep="")
  sink(outfilePY)
    ...
    for(lon in seq(lon_min, lon_max, 1)){
      ...
      if(abs(lat)<55){
        ...
        if( !is.na(test[,1]) ){ #### if tile and boundary overlap
          ...
          r <- "--root_path /gpfs/data1/duncansongp/gedi_data/l4a"
          f <- "-f /gpfs/data1/duncansongp/GEDI_global_PA/txt/gedi04_a_filelist.txt"
          d <- paste("-d /gpfs/data1/duncansongp/GEDI_global_PA/txt/",GEDI_data,"_vars.txt", sep="")
          b <- paste("-b", xmin, ymax, xmax, ymin, sep=" ")
          output <- paste(f.path,"WDPA_gedi_",GEDI_data,"/",iso3,"/",file_out,sep="")
          cat(paste(py, r, f, d, b, output, sep=" "))
          cat("\n")
        }
      }
    }
