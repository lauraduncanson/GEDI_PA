...
## l2a
################################################################################
#startT <- "--start_time 2020-04-16"  ### day of year 107 (newdata2) ### day of year 051 (newdata1)
#endT <- "--end_time 2020-09-02"  ### day of year 246 (newdata2) ### day of year 106 (newdata1)
GEDI_data <- "l2a"
...
for(i in 1:length(IDs)){
  ...
  roi_name <- iso3
  outfilePY <- paste(f.path,"WDPA_countries/py_",GEDI_data,"_newdata2/",iso3,".py",sep="")
  sink(outfilePY)
    ...
    for(lon in seq(lon_min, lon_max, 1)){
      ...
      if(abs(lat)<55){
        ...
        if( !is.na(test[,1]) ){ #### if tile and boundary overlap
          ...
          d <- paste("-d /gpfs/data1/duncansongp/leitoldv/txt/",GEDI_data,"_vars2.txt", sep="")
          b <- paste("-b", xmin, ymax, xmax, ymin, sep=" ")
          etc <- "--product GEDI02 --level A --pgeversion 1 --type 02"
          output <- paste(f.path,"WDPA_gedi_",GEDI_data,"/",iso3,"/",file_out,sep="")
          cat(paste(py, r, d, b, etc, output, sep=" "))
          cat("\n")
        }
      }
    }
