...
PART0a. Subset GEDI data to Brazil extent; extract L2A variables
```{python}
...
PART0b. Filter out 'bad quality' GEDI data
```{r}
...
PART1. Create 1km sampling grid with points only where GEDI data is available
```{r}
library(sp)
library(rgdal)
library(raster)
...
PART2. Clip the sampling grid to testPA and nonPA areas within country
```{r}
f.path <- "/gpfs/data1/duncansongp/leitoldv"
