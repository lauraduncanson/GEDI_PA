...
#2) clip the rasters to Brazil adm boundary
...
STEP2. Clip the sampling grid to testPA and nonPA areas within country
```{r}
f.path <- "/gpfs/data1/duncansongp/leitoldv"

GRID.for.matching <- readRDS(file.path(f.path,"brazil_PAs/BRA_grid/GRID.for.matching.RDS"))

#4) read in protected areas for selected country from WDPA dataset
allPAs <- readOGR(file.path(f.path,"brazil_PAs/BRA_pa/WDPA_Jun2020_BRA-shapefile-polygons.shp"))

#5) clip GRID.for.matching extent to testPA area
testPA <- allPAs[allPAs$NAME=="Kayapó",]
testPA <- spTransform(testPA, "+init=epsg:4326")
GRID.pts.testPA <- GRID.for.matching[testPA]
print(length(GRID.pts.testPA))
saveRDS(GRID.pts.testPA, file = file.path(f.path,"brazil_PAs/BRA_grid/GRID.pts.testPA.RDS")) 

#6) select each PA, buffer it by 1km, erase GRID points that overlap with PA
GRID.pts.nonPA <- GRID.for.matching
for(i in 1:length(allPAs)){
  PA          <- allPAs[i,]
  PA_prj      <- spTransform(PA, "+init=epsg:5880")
  PA_prj_buff <- gBuffer(PA_prj, width = 1000) #1km buffer
...
STEP3. Sample raster layers at 1 km GEDI grid points in testPA and nonPA areas
```{r}
f.path <- "/gpfs/data1/duncansongp/leitoldv"

### read in Brazil testPA and nonPA sampling grids
GRID.pts.testPA <- readRDS(file.path(f.path,"brazil_PAs/BRA_grid/GRID.pts.testPA.RDS"))
...
d_com <-d_all[complete.cases(d_all), ]  #filter away non-complete cases w/ NA, only 70% of the data
#calculate the propensity scores & filter out controls not overlapping w/ treatment propensity scores
ps <- glm(status ~ mean_temp + prec + elevation + slope+ gHMI,data = d_com)
...
d_com$propensity_score <- fitted(ps)
d_sep <- d_com %>% group_by(status)
d_sep_range <- d_com %>% group_by(status)%>% 
  summarise(propmin= min(propensity_score), promax=max(propensity_score)) 
print(d_sep_range)
#  status  propmin promax
#    <dbl>    <dbl>  <dbl>
# 1      0 -0.0216  0.108 
# 2      1  0.00561 0.0570
d_filtered <- d_sep %>% 
  filter(status==1 | between(propensity_score,d_sep_range$propmin[2],d_sep_range$promax[2])) %>% 
  ungroup() #59% percent data left
#check balance score prior to matching
pre <-   xBalance(status ~ mean_temp + prec + elevation + slope+ gHMI, 
                  data =d_filtered,
                  report = c("adj.means", "std.diffs",
                             "z.scores", "chisquare.test"))  #prior to matching chisqure 31033
print(pre$overall) #chisquare 6865
# plot(pre)
