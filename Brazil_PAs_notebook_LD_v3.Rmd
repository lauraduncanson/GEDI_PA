...
for(i in 1:length(dir())){
  ...
  gedi_data  <- read.csv(list.files(gedi_folder,full.names=T)[i]) %>%
    dplyr::select(lon_lowestmode,lat_lowestmode,rh_098,rh_075,rh_050,rh_025,shot_number)
  gedi_pts     <- SpatialPointsDataFrame(coords=gedi_data[,c("lon_lowestmode","lat_lowestmode")], 
                                         proj4string=CRS("+init=epsg:4326"), data=gedi_data)
  gedi_pts_prj <- spTransform(gedi_pts, "+init=epsg:6933")
  gedi_pixel <- SpatialPixelsDataFrame(gedi_pts_prj, tolerance = 0.999, gedi_pts_prj@data)
  gedi_ras < raster(gedi_pixel[,c('rh_098')])
  crs(gedi_ras) <- CRS("+init=epsg:6933")
  ...
  GRID.coords_sp <- SpatialPoints(coords = GRID.coords,proj4string=CRS("+init=epsg:6933"))
  grid_with_rh <- raster::extract(gedi_ras,GRID.coords_sp)
  print(nrow(GRID.coords@coords))
}
...
results = foreach(this_pa=d_PAs,.combine = foreach_rbind, .packages=c('sp','magrittr', 'dplyr','tidyr','optmatch','doParallel')) %dopar% {
  pa=this_pa
  id_pa <-this_pa %>%gsub("[^0-9]", "",  .)
  
  d_pa <- readRDS(this_pa)
  d_filtered_prop <- propensity_filter(d_pa)  #return a df of control and treatment after complete cases and propensity filters are applied 
  
  #treatment dataset
  d_wocat_all <- filter(d_filtered_prop, status)
  #control dataset
  d_control_all <- filter(d_filtered_prop, !status)
  ...
  t <- ifelse(round(n_control/n_treatment)<=7, round(n_control/n_treatment),7)
  ...
  match_score <- tryCatch(m_all2_out %>%dplyr::mutate(., id = row_number()) %>% 
                            ...
                            dplyr::rename(postmatch_chisquare=m_all2.match_obj_chisquare,postmatch_df=m_all2.match_obj_df,postmatch_pvalue=m_all2.match_obj_p.value,prematch_chisquare= Unadj_chisquare, prematch_df= Unadj_df, prematch_pvalue=Unadj_p.value), 
                          error=function(e) return(NULL))
...
write.csv(results,"matching_outcome_balancescores.csv" )
```
