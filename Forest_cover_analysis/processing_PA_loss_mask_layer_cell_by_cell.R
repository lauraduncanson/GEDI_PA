...
getLossAreaPerPA <- function(iso3) {
  ...
  match_results <- readRDS( paste(rds.f.path,iso3,"_PA.RDS",sep="")) 
  ...
  for(i in 1:length(tiles_processed)){
    if (file.exists(paste(write_f.path, iso3,"/",iso3,"_PA_with_dup_", tiles_processed[i],".rds",sep=""))){
       print(paste("file ", write_f.path, iso3,"/",iso3,"_PA_with_dup_", tiles_processed[i],".rds", " exists",sep=""))
    } else{
  ...
  foreach(this_r=unique(tiles_to_process),.combine = rbind, .packages=c('raster','sp','magrittr', 'dplyr','tidyr','optmatch','doParallel')) %dopar% {
    ...
    if(!is.null(sampleloss)){
      ...
      saveRDS(gridPolygonDF,paste(write_f.path, iso3,"/",iso3,"_PA_with_dup_", this_r,".rds",sep=""))
...
mcmapply(FUN = getLossAreaPerPA,country_list, mc.cores = 10) #mc.cores = 10 -> processing 10 countries at a time, which can be adapted based on the system used to process
